...
int main(int argc, char** argv)
{
  ...
  if (argc == 4)
  {
    try 
    {
      stop_limit = boost::bad_lexical_cast<size_t>(argv[3]);
    }
    catch (const boost::bad_lexical_cast&)
    {
      std::cerr << "genaddr: Bad RANGSTOP provided" << std::endl;
      return -1;
    }
  }
  if (stop_limit < n)
  {
    std::cerr << "genaddr: RANGSTOP cannot be less than N" << std::endl;
  }
