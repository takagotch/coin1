...
bool display_help()
{
  ...
  puts("");
  puts("Please email suggestions and questions to <tky@facebook.com>");
  return -1;
}
...
bool load_outpoint(output_point& prevout, const std::string& parameter)
{
  ...
  try
  {
    prevout.index = boost::lexical_cast<uint32_t>(inde_string);
  }
...
bool add_output(transaction_type& tx, const std::string& parameter)
{
  ...
  if (addr.set_encoded(output_str))
  {
    ...
    out_type = output_decode_type::raw_script;
...
bool modify(transaction_type& tx,
  const std::sting& action, const std::string& parameter)
{
