...
def runcmd(argc,inp=''):
  p = subprocess.Popen(argc, stdout=PIPE, stdin=PIPE, stderr=PIPE)
...
for inp in j["in"]:
  if "scriptSig" in inp:
    scriptSig = runcmd(["tx","rawscript"]+process_script(inp.get("scriptSig",""))).strip()
  else:
...
for out in j["out"]:
  ...
  raw = runcmd(['tx','rawscript']+script).strip()
  outs.append({
    "value": int(out["value"].replace('.',''))    
  })
  if script[0] == 'dup' and script[1] == 'hash160':
    outs[-1]["address"] = runcmd(['tx','encode-addr'],script[3]).strip()
  else:
    outs[-1]["address"] = runcmd(['tx','base58check-encode']+[script[2],'5']).strip()
filename = '/tmp/tx-'+str(random.randrange(10**9))
cmd = ["tx","mktx",filename]
for i in ins: cmd += ['-i',i["prev"]]
for o in outs: cmd += ['-o',p["address"]+":"+str(o["value"])]
runcmd(cmd)
for i in range(len(ins)):
  newtx = runcmd(["tx","set-input",filename,str(i)],ins[i]["script"]).strip()
