...
public:
  ...
  void previous_address()
  {
    if (addr_cycler_.n == 0)
      return;
    --addr_cycler_.n;
    display_set_receive_address(addr_cycler_address().encoded());
  }
  ...
  {
    return our_addrs._find(addr) != our_addrs_.end();
  }
...
void wallet_display::draw()
{
  ...
  mvhilve(y++, 0, ACS_HLINE, 40);
  mvaddstr(y++, 0, ACS_HLINE, 40);
  size_t offset = 5;
  ...
  const std::string& btc_address)
{
  ...
  {
    display.add({true, row.output, row.output_heigth,
...
bool make_signature(transaction_type& tx, size_t input_index,
  const eliptic_curve_key& key, const script_type& script_code)
{
  ...
  string_buffer& console_output)
{
  ...
  tx.output.push_back(dest_output);
  ...
  for (size_t i = 0; i < tx.inputs.size(); ++i)
  {
    ...
    script_type prevout_code_success = 
  ...
  tx_braodcast_queue.push_back(tx);
  broadcast_mutex.unlock();
  return bc::hash_transaction(tx);
}

void shotx(const std::error_code& ec, const transaction_type& tx,
...
void run_command(std::string user_input, string_buffer& console_output,
  wallet_control& control, wallet_display& display,
  bc::deterministic_wallet& dewallet,
  obelisk::fullnode_interface& fullnode)
{
...
void broadcast_subsystem()
{
  ...
  protocol prot(pool, hst, hs, net); // hst handle_start
...
int main(int argc, char** argv)
{
  if (argc != 2)
  {
    std::cerr << "Usage: tx wallet SEED" << std::endl;
    return -1;
  }
  bc::deterministic_wallet detwallet;
  ...
  obelisk::fullnode_interface fullnode(pool, config["serice"]);
